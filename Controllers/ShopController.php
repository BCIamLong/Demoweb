...
class ShopController
{
    ...
    function list()
    {
        ...
        $data_loaisp2 = $this->shop_model->loaisp2(0, 8);
        $data_loaisp3 = $this->shop_model->loaisp3(0, 8);
        ...
        if (isset($_GET['sp']) and isset($_GET['loai'])) {
           ...
           if(isset($_GET['sapxep'])){
            if(isset($_GET['MaLSP'])){
                ...
                $data = $this->shop_model->sanpham_sapxeploai($_GET['MaLSP']);
                $data_noibat = $this->shop_model->sanpham_noibat();
               $data_tong = count($data);
           }else{
             $data = $this->shop_model->sanpham_sapxep();
                $data_noibat = $this->shop_model->sanpham_noibat();
               $data_tong = count($data);
           }
