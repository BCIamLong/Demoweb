...
class model
{
    ...
    function sanpham_danhmuc($a, $b, $danhmuc)
    {
      ...
      function sanpham_sapxep()
    {
        $query =   "SELECT * from sanpham WHERE TrangThai = 1 ORDER BY DonGia DESC ";

        require("result.php");
        
        return $data;
    }
     function sanpham_sapxeploai($ten)
    {
        $query =   "SELECT * from sanpham WHERE MaLSP=$ten ORDER BY DonGia DESC ";

        require("result.php");
        
        return $data;
    }
    function sanpham_loai($a, $b, $loai)
    {
        $query =   "SELECT * from sanpham WHERE TrangThai = 1  and MaLSP = $loai ORDER BY ThoiGian DESC limit $a,$b";
